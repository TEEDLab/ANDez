"""
This is a substantially modified version of the code from the BEARD library 
...
def clusterf_precision_recall_fscore(labels_true, labels_pred):
    ...
    true_keys.sort()
    T = [list(true_clusters[i]) for i in true_keys]

    pred_keys = list(pred_clusters.keys())
    pred_keys.sort()
    P = [list(pred_clusters[i]) for i in pred_keys]
    ...
    for i, P_i in zip(range(len(P)),P):
        for p in P_i:
            pIndex[p] = i + 1

        cSize[i + 1] = len(P_i)        
        pairPrSum += len(P_i)*(len(P_i) - 1)/2
		
    instSum = 0

    for T_j in T:
        instSum += len(T_j)
        pairTrSum += len(T_j)*(len(T_j) - 1)/2
        tMap = {}
        maxKey = 0
        maxValue = 0
        ...
        for key, value in sorted(tMap.items(), key = lambda kv: kv[0]):
            if value == len(T_j) and cSize[key] == len(T_j):
                cMatch += 1   
            aapSum += pow(value,2)/len(T_j)
            ...
            spSum += (len(T_j) - maxValue)  
            lmSum += (cSize[maxKey] - maxValue)  
	
    recall = cMatch/len(T)
    precision = cMatch/len(P)
...
def split_lump_error_precision_recall_fscore(labels_true, labels_pred):
    ...
    for i, P_i in zip(range(len(P)),P):
        ...
        cSize[i + 1] = len(P_i)         # hash of a cluster P_i and its size
        pairPrSum += len(P_i)*(len(P_i) - 1)/2
    ...
    # Compute Splitting & Lumping Error
    recall = 1 - SE
    precision = 1 - LE
